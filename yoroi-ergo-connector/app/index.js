      ...
      ergo.get_balance().then(async function (result) {
        ...
        button.onclick = async function () {
          ...
          const utxos = await ergo.get_utxos(
            amountToSend + wasm.TxBuilder.SUGGESTED_TX_FEE().as_i64().as_num()
          );
          //alert(utxos.map(utxo => parseInt(utxo.value)));
          const utxosValue = utxos.map(utxo => parseInt(utxo.value, 10)).reduce((a, b) => a + b, 0);
          console.log(
            `${utxosValue} - ${amountToSend} - ${wasm.TxBuilder.SUGGESTED_TX_FEE()
              .as_i64()
              .as_num()}`
          );
          const changeValue =
            utxosValue - amountToSend - wasm.TxBuilder.SUGGESTED_TX_FEE().as_i64().as_num();
          ...
          const boxSelection = selector.select(
            ...
            wasm.BoxValue.from_i64(
              amountToSendBoxValue.as_i64().checked_add(wasm.TxBuilder.SUGGESTED_TX_FEE().as_i64())
            ),
            new wasm.Tokens()
          );
          ...
          ergo
            ...
            .catch(err => {
              status.innerText =
                'You must accept signing the transaction to donate. Please click send again and accept.';
            });
        };
