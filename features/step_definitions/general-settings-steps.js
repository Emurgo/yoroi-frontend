...
When(/^I select second theme$/, async function () {
  await this.click('.DisplaySettingsBlock_themesWrapper > button:nth-child(2)');
});
...
Then(/^I should see second theme as selected$/, async function () {
  await this.waitForElement('.DisplaySettingsBlock_themesWrapper button:nth-child(2).DisplaySettingsBlock_active');
});
