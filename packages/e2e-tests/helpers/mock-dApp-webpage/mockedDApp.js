...
export class MockDAppWebpage {
  ...
  async _requestAccess(auth = false) {
    this.logger.info(`MockDApp::_requestAccess Requesting the access ${auth ? 'with' : 'without'} authentication`)
  ...
  async submitTx(signedTxHex) {
    ...
    this.logger.info(`MockDApp::getBalance The response is ${JSON.stringify(submitResponse, null, 2)}`)
