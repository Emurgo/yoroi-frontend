...
import {createTestRunDataDir, getByLocator, getTargetBrowser} from '../utils.js'
import {getExtensionUrl} from '../driverBootstrap.js'
import {TargetBrowser} from '../helpers/constants.js'
import {defaultRepeatPeriod, defaultWaitTimeout, halfSecond, oneSecond} from '../helpers/timeConstants.js'
...
class BasePage {
  ...
  async input(locator, value) {
    this.logger.info(`BasePage::input is called. Locator: ${JSON.stringify(locator)}, Value: ${value}`)
    const input = await this.findElement(locator)
    await input.sendKeys(value)
  }
  async inputElem(webElement, value) {
    this.logger.info(`BasePage::inputElem is called. WebElement: ${JSON.stringify(webElement)}, Value: ${value}`)
