...
describe('Connect Trezor HW wallet', function () {
  ...
  it('Check new wallet', async function () {
    ...
    expect(walletInfo.walletBalance, 'The wallet balance is different')
      .to
      .equal(testWalletTrezor.balance)
    expect(walletInfo.walletName, `The wallet name should be "${testWalletTrezor.name}"`)
      .to
      .equal(testWalletTrezor.name)
    expect(walletInfo.walletPlate, `The wallet plate should be "${testWalletTrezor.plate}"`)
      .to
      .equal(testWalletTrezor.plate)
  })
