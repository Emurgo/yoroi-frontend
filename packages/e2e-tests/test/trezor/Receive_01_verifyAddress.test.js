...
for (const model in TrezorModels) {
  ...
  describe(`Verify address on Trezor ${model}`, function () {
    ...
    before(async function () {
      ...
      trezorPage = new TrezorConnect(webdriver, logger);
    ...
    it('Approve connection', async function () {
      ...
      await trezorPage.tickCheckbox();
      await trezorPage.allowConnection();
      await trezorPage.allowPubKeysExport();
    ...
    it('Verify address', async function () {
      ...
      const exportPageIsDisplayed = await trezorPage.exportAddressPageIsDisplayed();
      expect(exportPageIsDisplayed, 'The Trezor Export Address page is not displayed').to.be.true;
      await trezorPage.confirmExportAddress();
