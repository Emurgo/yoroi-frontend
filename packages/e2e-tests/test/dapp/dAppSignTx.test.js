...
import {customAfterEach, customBeforeNestedDAppTest} from '../../customHooks.js'
...
describe('dApp, signTx, intrawallet Tx', function () {
  ...
  describe('[nested-dapp] Positive case', function () {
    ...
    it('Checking info on Details page', async function () {
      ...
      expect(displayedFeeNumber, 'Displayed fee is different from expected').to.equal(storedFeeNumber)
      // because it is intrawallet tx only fee + 0 is displayed in the total amount
      expect(displayedTotalNumber, 'Displayed total is different from expected').to.equal(storedFeeNumber)
    })
    ...
    it('Checking info on UTxOs page', async function () {
      ...
      const filteredAddrs = outputsInfo.yourAddrs.filter((addrInfo) => addrInfo.addr === receiverAddr)
      expect(filteredAddrs, 'Receiver address is not in outputs').to.be.an('array').that.is.not.empty
      ...
      expect(filteredAddrAssets[0].tokenAmount, 'Different amount for the receiver address').to.equal(1)
    })
