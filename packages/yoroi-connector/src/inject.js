...
const CATALYST_API_VERSION = 'draft';
...
(() => {
  ...
  async function enable_catalyst(purposes) {
    class CatalystAPI {
      constructor(rpc) {
    ...
    if (!await cardano_check_read_access()) {
      return Promise.reject({ code: -1, info: "API not enabled" });
    }

    const response = await cardano_rpc_call('enable_catalyst', [purposes]);
    if (response) {
      return new CatalystAPI(cardano_rpc_call);
    }
    return Promise.reject({ code: 2, info: "User declined" });
  }
  ...
  window.cardano = {
    ...
    '${WALLET_NAME}': {
      ...
      name: '${WALLET_NAME}',
      catalyst: {
        apiVersion: '${CATALYST_API_VERSION}',
        enable: enable_catalyst,
      },
    }
  };
})();
