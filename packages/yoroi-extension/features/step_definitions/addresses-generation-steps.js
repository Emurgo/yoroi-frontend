...
When(/^I click on the top-level address book tab$/, async function () {
  await this.click({ locator: `//div[contains(text(), "Address book") and contains(@class, "ReceiveNavButton_label")]`, method: 'xpath' });
});

When(/^I click on the top-level reward address tab$/, async function () {
  await this.click({ locator: `//div[contains(text(), "Reward") and contains(@class, "ReceiveNavButton_label")]`, method: 'xpath' });
});
...
When(/^I click on the Generate new address button ([0-9]+) times$/, async function (times) {
  for (let curr = 1; curr <= times; curr++) {
    await this.click({ locator: '.generateAddressButton', method: 'css' });
    await this.waitForElement({ locator: `.generatedAddress-${curr + 1} .RawHash_hash`, method: 'css' });
  }
});

Then(/^I should see my latest address "([^"]*)" at the top$/, async function (address) {
  await this.waitUntilText({ locator: '.StandardHeader_copyableHash', method: 'css' }, truncateAddress(address));
});
...
Then(/^I should see the addresses exactly list them$/, async function (table) {
  ...
  const waitUntilAddressesAppeared = rows.map((row, index) => (
    this.waitUntilText(
      {
        locator: `.generatedAddress-${index + 1} .RawHash_hash`,
        method: 'css'
      },
  ...
  const noMoreAddressAppeared = this.waitForElementNotPresent({
    locator: `.generatedAddress-${rows.length + 1} .RawHash_hash`,
    method: 'css'
    });
...
Then(/^I shouldn't see the address "([^"]*)"$/, async function (address) {
  await this.waitForElementNotPresent({
    locator: `//div[contains(text(), "${truncateAddressShort(address)}")]`,
    method: 'xpath'
  });
});
...
Then(/I should see an error about max unused addresses/, async function () {
  ...
  await this.waitUntilText({ locator: '.StandardHeader_error', method: 'css' }, errorMessage);
});
