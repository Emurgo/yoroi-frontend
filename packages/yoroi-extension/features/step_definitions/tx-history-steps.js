...
Then(/^I should see ([^"]*) ([^"]*) transactions$/, async function (txsNumber, txExpectedStatus) {
  ...
  for (let i = 1; i < txsAmount; i++) {
    const buttonShowMoreExists = await this.checkIfExists({
      locator: showMoreLocator,
      method: 'css',
    });
  ...
  const pendingTxsList = await this.getElementsBy({
    locator: '.Transaction_pendingLabel',
    method: 'css',
  });
  const failedTxsList = await this.getElementsBy({
