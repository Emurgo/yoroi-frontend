...
import LocalStorageApi, { loadSubmittedTransactions } from '../../../../../app/api/localStorage';
...
type HandleFuncType<WalletType, ParamT, ReturnT> = (request: RequestType<WalletType, ParamT>) => Promise<ReturnType<ReturnT>>;
...
class NewHandler {
  static basic<ParamT, ReturnT>(handle: HandleFuncType<void, ParamT, ReturnT>): Handler<ParamT, ReturnT> {
    return { needConnectedWallet: false, handle };
  }

  static withWallet<ParamT, ReturnT>(handle: HandleFuncType<PublicDeriver<>, ParamT, ReturnT>): Handler<ParamT, ReturnT> {
    return { needConnectedWallet: true, syncConnectedWallet: false, handle };
  }

  static withSyncedWallet<ParamT, ReturnT>(handle: HandleFuncType<PublicDeriver<>, ParamT, ReturnT>): Handler<ParamT, ReturnT> {
    return {
      needConnectedWallet: true,
      syncConnectedWallet: true,
      handle: request => checkWalletIsStillConnected(handle, request),
    };
  }
}

const checkWalletIsStillConnected = async <ParamT, ReturnT, WalletType>(
  handle: HandleFuncType<WalletType, ParamT, ReturnT>,
  request: RequestType<WalletType, ParamT>
): Promise<ReturnType<ReturnT>> => {
  const { message } = request;
  const whitelistedEntry = await findWhitelistedConnection(message.url, false, new LocalStorageApi());
...
const Handlers = Object.freeze({
  'is_enabled/cardano': NewHandler.basic<void, boolean>(async ({ message, tabId }) => {
    try {
      const whitelistedEntry = await findWhitelistedConnection(message.url, false, new LocalStorageApi());
  ...
  get_balance: NewHandler.withSyncedWallet<[string /* tokenId */], string | /* cbor */ (AccountBalance | Value) /* non-cbor */>(
    async ({ message, wallet }) => {
      ...
      if (message.returnType === 'cbor' && tokenId === '*' && !(typeof balance === 'string')) {
        ...
        const value = W4.Value.new(W4.BigNum.from_str(balance.default));
  ...
  get_used_addresses: NewHandler.withWallet<[boolean /* paginate */], Array<Address> | /* cbor */ Array<string> /* non-cbor */>(
    async ({ wallet, message }) => {
  ...
  get_unused_addresses: NewHandler.withWallet<void, Array<Address> | /* cbor */ Array<string> /* non-cbor */>(
    async ({ wallet, message }) => {
  ...
  'get_reward_addresses/cardano': NewHandler.withWallet<void, Array<Address> | /* cbor */ Array<string> /* non-cbor */>(
    async ({ wallet, message }) => {
  ...
  get_drep_key: NewHandler.withWallet<void, string>(async ({ wallet }) => {
    const dRepKey = await connectorGetDRepKey(wallet);
    return { ok: dRepKey };
  }),

  get_stake_key: NewHandler.withWallet<void, {| key: string, isRegistered: boolean |}>(async ({ wallet }) => {
    const stateFetcher: CardanoIFetcher = await getCardanoStateFetcher(new LocalStorageApi());
    const resp = await connectorGetStakeKey(wallet, stateFetcher.getAccountState);
    return { ok: resp };
  }),

  get_change_address: NewHandler.withWallet<void, string>(async ({ wallet, message }) => {
  ...
  submit_tx: NewHandler.withWallet<[string /* tx hex */], string>(async ({ wallet, message }) => {
    const txHex = message.params[0];
    await connectorSendTxCardano(wallet, txHex, new LocalStorageApi());
    try {
      await connectorRecordSubmittedCardanoTransaction(wallet, txHex);
    } catch (error) {
  ...
  'create_tx/cardano': NewHandler.withWallet<[CardanoTxRequest], string>(async ({ wallet, message, tabId }) => {
    const connection = await getConnectedSite(tabId);
    if (connection == null) {
      // shouldn't happen
      Logger.error(`ERR - sign_tx could not find connection with tabId = ${tabId}`);
  ...
  get_network_id: NewHandler.withWallet<void, number>(async ({ wallet, tabId }) => {
    const connection = await getConnectedSite(tabId);
    if (connection == null) {
      // shouldn't happen
  ...
  'list_nfts/cardano': NewHandler.withSyncedWallet<void, {| [string]: {| metadata: NFTMetadata | null |} |}>(
    async ({ wallet }) => {
  ...
  'auth_sign_hex_payload/cardano': NewHandler.basic<[string], string>(async ({ tabId, message }) => {
    const connection = await getConnectedSite(tabId);
  ...
  'auth_check_hex_payload/cardano': NewHandler.basic<[string, string], boolean>(async ({ tabId, message }) => {
    const connection = await getConnectedSite(tabId);
    if (!connection) {
    ...
    const pk = RustModule.WalletV4.PublicKey.from_hex(String(connection.status?.auth?.pubkey));
  ...
  get_collateral_utxos: NewHandler.withSyncedWallet<string, void>(async ({ wallet, tabId, message }) => {
    ...
    const { utxosToUse, reorgTargetAmount } = await connectorGetCollateralUtxos(
      wallet,
      requiredAmount,
    ...
    // do have enough
    if (canAnswer) {
      const utxos = await transformCardanoUtxos(utxosToUse, message.returnType === 'cbor');
      return {
        ok: utxos,
      };
    }
    ...
    if (reorgTargetAmount != null) {
      ...
      const connection = await getConnectedSite(tabId);
      if (connection == null) {
        throw new Error(`ERR - get_collateral_utxos could not find connection with tabId = ${tabId}`);
      }
...
export async function handleRpc(message: Object, sender: Object) {
  ...
  try {
    ...
    if (error instanceof ConnectorError) {
      sendRpcResponse({ err: error.toAPIError() }, tabId, message.uid);
    } else {
      ...
      Logger.error(info);
      sendRpcResponse({ err: { code: APIErrorCodes.API_INTERNAL_ERROR, info } }, tabId, message.uid);
    }
  }
}
