...
import { genCSP } from './constants-mv2';
import { Servers, serverToPermission } from '../scripts-mv2/connections';
...
export default (isDebug: boolean, shouldInjectConnector: boolean): * =>
  buildManifest({
    ...
    contentSecurityPolicy: genCSP({
      isDev: isDebug,
      additional: {
        'connect-src': [serverToPermission(Servers.Primary), serverToPermission(Servers.Testnet)],
        'frame-src': [POOLS_UI_URL_FOR_YOROI, CHANGELLY_URL],
      },
    }),
    version: pkg.version,
