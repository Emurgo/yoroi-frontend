...
import type { DeviceCodeType, OperationNameType } from '../../../types/enum';
...
export default class DeviceSelectionBlock extends React.Component<Props> {
  ...
  render(): Node {
    ...
    const { knownDeviceCode, currentOperationName } = this.props;
      ...
      case DEVICE_CODE.NONE:
        middleComp = (
          <div className={styles.deviceSelection}>
            <div className={styles.title}>{intl.formatMessage(message.chooseDevice)}</div>
            <button
            ...
            >
              <div className={styles.text}>{intl.formatMessage(message.deviceNanos)}</div>
            </button>
            <button
              className={styles.button}
              type="button"
            ...
            >
              <div className={styles.text}>{intl.formatMessage(message.deviceNanox)}</div>
            </button>
          </div>
        );
        break;
          ...
          middleComp = (
            <div className={styles.deviceKnown}>
              <div className={styles.knowInfoBlock}>
                ...
                <div className={styles.knownInfoText}>{intl.formatMessage(message.knownInfo2)}</div>
                <div className={styles.knownInfoText}>
                  <FormattedMessage {...message.knownInfo3} values={{ strong }} />
                </div>
              </div>
              <div className={styles.continueButtonBlock}>
                <button className={styles.button} type="button" onClick={this.onExecuteActionClicked.bind(this, knownDeviceCode)}>
                  <div className={styles.text}>{intl.formatMessage(message.knownContinueButtonText)}</div>
                </button>
              </div>
              <div className={styles.knownChoseWrongBlock}>
                <span className={styles.knownChoseWrongText}>{intl.formatMessage(message.knownChoseWrongDevice)}</span>
                <button className={styles.linkClickHere} type="button" onClick={this.onDeviceSelectionClicked.bind(this)}>
    ...
    return (
      <div className={styles.component}>
        ...
        <div className={styles.videoLink}>
          <span className={styles.videoLinkText}>{intl.formatMessage(message.videoLinkPart1)}</span>
          <a
          ...
          <img className={styles.linkIcon} src={imgExternalLink} alt="External Link" />
          <span className={styles.videoLinkText}>{intl.formatMessage(message.videoLinkPart2)}</span>
          <a
          ...
          <img className={styles.linkIcon} src={imgExternalLink} alt="External Link" />
          <span className={styles.videoLinkText}>{intl.formatMessage(message.videoLinkPart3)}</span>
        </div>
      </div>
    );
  }
