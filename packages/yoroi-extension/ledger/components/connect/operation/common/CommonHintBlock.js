...
import type { DeviceCodeType, ProgressStateType } from '../../../../types/enum';
...
const message = defineMessages({
  sPinCode: {
    ...
    defaultMessage:
      '!!!If your Ledger device is locked enter your PIN, using the <strong>right</strong> and <strong>left</strong> buttons to select each number and then <strong>both</strong> buttons to confirm.',
  },
  sCardanoApp: {
    ...
    return (
      <div className={styles.component}>
        <div className={styles.stepsRow}>
          <HintBlock number={1} text={message[`${deviceCode}PinCode`]} imagePath={imgCommon1} />
          <HintGap />
          <HintBlock number={2} text={message[`${deviceCode}CardanoApp`]} imagePath={imgCommon2} />
        </div>
      </div>
    );
  }
