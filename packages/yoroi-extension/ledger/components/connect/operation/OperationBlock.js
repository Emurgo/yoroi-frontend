...
import type { DeviceCodeType, OperationNameType, ProgressStateType } from '../../../types/enum';
import { OPERATION_NAME, PROGRESS_STATE } from '../../../types/enum';
  ...
  render(): Node {
    ...
    switch (progressState) {
      ...
      case PROGRESS_STATE.DETECTING_DEVICE:
        content = <CommonHintBlock deviceCode={deviceCode} progressState={progressState} />;
        break;
      case PROGRESS_STATE.DEVICE_FOUND:
        ...
        switch (currentOperationName) {
          case OPERATION_NAME.GET_EXTENDED_PUBLIC_KEY:
            content = <ConnectLedgerHintBlock deviceCode={deviceCode} wasDeviceLocked={wasDeviceLocked} />;
            break;
          case OPERATION_NAME.GET_EXTENDED_PUBLIC_KEYS:
            content = <ConnectLedgerMultiKeysHintBlock deviceCode={deviceCode} wasDeviceLocked={wasDeviceLocked} />;
            break;
          ...
          case OPERATION_NAME.SIGN_MESSAGE:
            content = (
              <SignMessageBlock deviceCode={deviceCode} wasDeviceLocked={wasDeviceLocked} deviceVersion={deviceVersion} />
            );
            break;
          default:
    ...
    if (showPerformActionText === true) {
      performActionText = <div className={styles.performActionText}>{intl.formatMessage(message.topInfo)}</div>;
    }
