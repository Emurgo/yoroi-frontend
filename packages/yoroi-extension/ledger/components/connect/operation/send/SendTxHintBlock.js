...
const message = defineMessages({
  ...
  sDelegation: {
    ...
    defaultMessage:
      '!!!Make sure the pool shown on your Ledger is the same as the one shown below, then press <strong>both</strong> buttons.',
  },
  sDelegationComplete: {
  ...
  xConfirmAddress: {
    ...
    defaultMessage:
      "!!!Confirm the receiver's address by pressing the <strong>right</strong> button to scroll through the entire address. Then press <strong>both</strong> buttons.",
  },
  xConfirmFee: {
    id: 'hint.sendTx.confirmFee',
    defaultMessage: '!!!Confirm Transaction Fee by pressing <strong>both</strong> buttons.',
...
export default class SendTxHintBlock extends React.Component<Props> {
  ...
  renderCertificate: ({| cert: Certificate, getAndIncrementStep: void => number, deviceVersion: ?string |}) => Array<Node> =
    request => {
      ...
      if (request.cert.type === CertificateType.STAKE_DELEGATION) {
        ...
        if (semverGte(request.deviceVersion, '2.4.1')) {
          poolId = bech32.encode('pool', bech32.toWords(Buffer.from(params.poolKeyHashHex, 'hex')));
        } else {
  ...
  render(): Node {
    const { deviceCode, signTxInfo, wasDeviceLocked, deviceVersion } = this.props;
    const stepStartNumber: number = wasDeviceLocked ? 2 : 0; // 2 = count of common step
    let stepNumber = stepStartNumber;
    let content;

    if (signTxInfo.tx.auxiliaryData && signTxInfo.tx.auxiliaryData.type === TxAuxiliaryDataType.CIP36_REGISTRATION) {
      ...
      content = (
        <>
          <HintBlock number={++stepNumber} text={message.catalystStep1} imagePath={imgStep1} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message[`${deviceCode}StartNewTx`]} imagePath={imgStep2} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message.catalystStep3} imagePath={imgStep3} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message.catalystStep4} imagePath={imgStep4} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message.catalystStep5} imagePath={imgStep5} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message.catalystStep6} imagePath={imgStep6} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message.catalystStep7} imagePath={imgStep7} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message.catalystStep8} imagePath={imgStep8} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message.catalystStep9} imagePath={imgStep9} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message[`${deviceCode}ConfirmFee`]} imagePath={imgStep10} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message[`${deviceCode}Ttl`]} imagePath={imgStep11} />
          <HintGap />

          <HintBlock number={++stepNumber} text={message[`${deviceCode}ConfirmTx`]} imagePath={imgStep12} />
          <HintGap />
        </>
      );
    } else {
      ...
      content = (
        <>
          <HintBlock number={++stepNumber} text={message[`${deviceCode}StartNewTx`]} imagePath={imgSend1} />
          <HintGap />
          {signTxInfo.tx.outputs.length > 0 &&
            signTxInfo.tx.outputs.map(output => {
              ...
              if (params.type === AddressType.BASE_PAYMENT_KEY_STAKE_KEY && params.params && params.params.stakingPath != null) {
                return null;
              }
              ...
              const result = [
                ...
                <HintBlock key={nextStep2} number={nextStep2} text={message[`${deviceCode}ConfirmValue`]} imagePath={imgSend2} />,
                <HintGap key={nextStep2 + 'gap'} />,
                ...(output.tokenBundle || []).flatMap(({ policyIdHex, tokens }) => {
                  return tokens.map(token => {
                    ...
                    return (
                      <>
                        ...
                        <HintBlock key={step2} number={step2} text={message.confirmAssetAmount} imagePath={imgTokenAmount} />
          ...
          <HintBlock number={++stepNumber} text={message[`${deviceCode}ConfirmFee`]} imagePath={imgSend4} />
          <HintGap />
          {signTxInfo.tx.ttl != null && <HintBlock number={++stepNumber} text={message[`${deviceCode}Ttl`]} imagePath={imgTtl} />}
          <HintGap />
          {signTxInfo.tx.certificates != null &&
            signTxInfo.tx.certificates.map(cert => this.renderCertificate({ cert, getAndIncrementStep, deviceVersion }))}
          ...
          {signTxInfo.tx.auxiliaryData != null && signTxInfo.tx.auxiliaryData.type === TxAuxiliaryDataType.ARBITRARY_HASH && (
            <>
              <HintBlock
                number={++stepNumber}
          ...
          <HintBlock number={++stepNumber} text={message[`${deviceCode}ConfirmTx`]} imagePath={imgSend5} />
          <HintGap />
        </>
      );
    }
    ...
    return (
      <div className={styles.component}>
        <div className={styles.stepsGrid}>{content}</div>
      </div>
    );
  }
}
