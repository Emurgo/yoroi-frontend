...
export function getAddressHintBlock(props: Props): Array<Node> {
  const { deviceCode, addressInfo } = props;
  ...
  if (addressInfo.type === AddressType.BYRON) {
    ...
    result.push(<HintBlock key={nextStep} number={nextStep} text={message[`${deviceCode}Warning`]} imagePath={byronWarning} />);
    result.push(<HintGap key={nextStep + 'gap'} />);
  }
  if (addressInfo.type === AddressType.ENTERPRISE_KEY) {
    const nextStep = props.getAndIncrementStep();
    result.push(
      <HintBlock key={nextStep} number={nextStep} text={message[`${deviceCode}Warning`]} imagePath={enterpriseWarning} />
    );
    result.push(<HintGap key={nextStep + 'gap'} />);
  }
  if (addressInfo.type === AddressType.REWARD_KEY) {
    const nextStep = props.getAndIncrementStep();
    result.push(<HintBlock key={nextStep} number={nextStep} text={message[`${deviceCode}Warning`]} imagePath={rewardWarning} />);
    result.push(<HintGap key={nextStep + 'gap'} />);
  }
  ...
  if (params.stakingBlockchainPointer != null) {
    ...
    result.push(
      <HintBlock
        ...
        imagePath={pointer}
        secondaryText={`(${stakingBlockchainPointer.blockIndex}, ${stakingBlockchainPointer.txIndex}, ${stakingBlockchainPointer.certificateIndex})`}
      />
    );
    result.push(<HintGap key={nextStep + 'gap'} />);
  }
