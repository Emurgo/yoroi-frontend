...
export function identifierToCardanoAsset(identifier: string): {|
...
export function parseTokenList(assets: void | RustModule.WalletV4.MultiAsset): Array<{|
  assetId: string,
  policyId: string,
  name: string,
  amount: string,
...
export function sumInputsOutputs(
  ios: $ReadOnlyArray<
    $ReadOnly<
      UtxoTransactionInputRow | UtxoTransactionOutputRow | AccountingTransactionInputRow | AccountingTransactionOutputRow,
    >,
  >,
...
function cardanoUtxoMonadFromRemoteFormat(
  u: RemoteUnspentOutput | CardanoAddressedUtxo
