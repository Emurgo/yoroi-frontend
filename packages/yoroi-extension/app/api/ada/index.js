...
export default class AdaApi {
    ...
    try {
      ...
      if (hasSendAllDefault(request.tokens)) {
        ...
        unsignedTxResponse = await shelleyNewAdaUnsignedTx(
          ...
          request.absSlotNumber,
          {
            ...
            linearFeeCoefficient: protocolParameters.linearFee.coefficient,
      ...
      return new HaskellShelleyTxSignRequest({
        ...
        networkSettingSnapshot: {
          ChainNetworkId: Number(getCardanoHaskellBaseConfig(request.network)[0].ChainNetworkId),
  ...
  async _createUnsignedTxForConnector(
    ...
    return new HaskellShelleyTxSignRequest({
      ...
      metadata: undefined,
      networkSettingSnapshot: {
        ChainNetworkId: Number(getCardanoHaskellBaseConfig(network)[0].ChainNetworkId),
        KeyDeposit: new BigNumber(protocolParameters.keyDeposit),
        PoolDeposit: new BigNumber(protocolParameters.poolDeposit),
        NetworkId: network.NetworkId,
      },
  ...
  async createDelegationTx(request: CreateDelegationTxRequest): Promise<CreateDelegationTxResponse> {
    ...
    try {
      ...
      const protocolParams = {
        ...
        linearFeeCoefficient: request.protocolParameters.linearFee.coefficient,
        linearFeeConstant: request.protocolParameters.linearFee.constant,
        coinsPerUtxoByte: request.protocolParameters.coinsPerUtxoByte,
        poolDeposit: request.protocolParameters.poolDeposit,
      ...
      const signTxRequest = new HaskellShelleyTxSignRequest({
        ...
        metadata: undefined,
        networkSettingSnapshot: {
          ChainNetworkId: Number(getCardanoHaskellBaseConfig(networkInfo)[0].ChainNetworkId),
  ...
  async cloneWallet(db: lf$Database, publicDeriver: PublicDeriver<>, network: $ReadOnly<NetworkRow>): Promise<PublicDeriver<>> {
    ...
    const cip1852Wallet = await createAndRefreshCip1852Wallet(db, wallet.cip1852WrapperRow);
    ...
    return await PublicDeriver.createPublicDeriver(pubDeriverResult, cip1852Wallet);
  }
  ...
  async restoreWallet(request: RestoreWalletRequest): Promise<RestoreWalletResponse> {
    ...
    try {
      ...
      const encryptedRoot = encryptWithPassword(walletPassword, rootPk.as_bytes());
      ...
      const cip1852Wallet = await createAndRefreshCip1852Wallet(request.db, wallet.cip1852WrapperRow);
  ...
  pickCollateralUtxo: ({| wallet: WalletState |}) => Promise<?QueriedUtxo> = async ({ wallet }) => {
    ...
    const smallPureUtxos = allUtxos.filter(
      u => u.output.tokens.length === 1 && utxoDefaultCoinAmount(u).lte(MAX_PICKED_COLLATERAL_UTXO_ADA)
    );
