...
import type { Cip1852WrapperInsert, Cip1852WrapperRow } from '../tables';
...
export class ModifyCip1852Wrapper {
  ...
  static async add(db: lf$Database, tx: lf$Transaction, request: Cip1852WrapperInsert): Promise<$ReadOnly<Cip1852WrapperRow>> {
  ...
  static async remove(db: lf$Database, tx: lf$Transaction, id: number): Promise<void> {
    const fullRow = await ModifyCip1852Wrapper.depTables.GetCip1852Wrapper.get(db, tx, id);
    if (fullRow == null) {
    ...
    await ModifyCip1852Wrapper.depTables.RemoveKeyDerivationTree.remove(db, tx, { rootKeyId: fullRow.RootKeyDerivationId });
  }
}
