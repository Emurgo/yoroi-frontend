...
import type { Cip1852WrapperRow } from '../tables';
...
export class GetCip1852Wrapper {
  ...
  static async get(db: lf$Database, tx: lf$Transaction, key: number): Promise<$ReadOnly<Cip1852WrapperRow> | void> {
...
export class GetAllCip1852Wallets {
  ...
  static async get(
    ...
    |}>,
  > {
    const publicDeriverTable = db.getSchema().table(GetAllCip1852Wallets.ownTables[PublicDeriverSchema.name].name);
    const Cip1852WrapperTable = db.getSchema().table(GetAllCip1852Wallets.ownTables[Tables.Cip1852WrapperSchema.name].name);
    const query = db
      .select()
