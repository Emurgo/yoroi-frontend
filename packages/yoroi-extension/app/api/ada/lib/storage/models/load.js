...
import { PublicDeriver } from './PublicDeriver/index';
...
export async function loadWalletsFromStorage(db: lf$Database): Promise<Array<PublicDeriver<>>> {
  const result = [];
  const depTables = getAllSchemaTables(db, GetAllCip1852Wallets);
  const cip1852WalletsInStorage = await raii(db, depTables, async tx => await GetAllCip1852Wallets.get(db, tx));
  // Cip1852
  {
    ...
    for (const entry of cip1852WalletsInStorage) {
      ...
      if (cip1852Wallet == null) {
        cip1852Wallet = await createAndRefreshCip1852Wallet(db, entry.Cip1852Wrapper);
        cip1852Map.set(entry.Cip1852Wrapper.ConceptualWalletId, cip1852Wallet);
      }
      const publicDeriver = await PublicDeriver.createPublicDeriver(entry.PublicDeriver, cip1852Wallet);
