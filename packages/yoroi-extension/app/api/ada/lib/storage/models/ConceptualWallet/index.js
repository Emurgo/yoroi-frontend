...
export class ConceptualWallet implements IConceptualWallet {
  ...
  rename: ({| newName: string |}) => Promise<void> = async body => {
    ...
    return await raii<void>(this.db, depTables, async tx => {
      await deps.ModifyConceptualWallet.rename(this.db, tx, {
  ...
  getFullConceptualWalletInfo: void => Promise<$ReadOnly<ConceptualWalletRow>> = async () => {
    ...
    const depTables = Object.keys(deps)
      .map(key => deps[key])
      .flatMap(table => getAllSchemaTables(this.db, table));
    return await raii<$ReadOnly<ConceptualWalletRow>>(this.db, depTables, async tx => {
      const row = await deps.GetConceptualWallet.get(this.db, tx, this.#conceptualWalletId);
      if (row == null) {
  ...
  rawRemove: (lf$Database, lf$Transaction) => Promise<void> = async (db, tx) => {
    await ModifyCip1852Wrapper.remove(db, tx, this.getConceptualWalletId());
    await rawRemoveConceptualWallet(db, tx, this.getConceptualWalletId());
  };
}

export async function rawRemoveConceptualWallet(db: lf$Database, tx: lf$Transaction, conceptualWalletId: number): Promise<void> {
  await ModifyConceptualWallet.remove(db, tx, [conceptualWalletId]);
}
function isHwKind(conceptualWallet: IConceptualWallet, matchKind: ($ReadOnly<HwWalletMetaRow>) => boolean): boolean {
...
export const isAnyTrezorWallet = (conceptualWallet: IConceptualWallet): boolean => {
  return isTrezorTWallet(conceptualWallet) || isTrezorSafe3Wallet(conceptualWallet) || isTrezorSafe5Wallet(conceptualWallet);
};

function isTrezorTWallet(conceptualWallet: IConceptualWallet): boolean {
  const tVendor = Config.wallets.hardwareWallet.trezorT.VENDOR;
  const tModel = Config.wallets.hardwareWallet.trezorT.MODEL;
  return isHwKind(conceptualWallet, hwWalletMeta => hwWalletMeta.Vendor === tVendor && hwWalletMeta.Model === tModel);
}

function isTrezorSafe3Wallet(conceptualWallet: IConceptualWallet): boolean {
  ...
  return isHwKind(conceptualWallet, hwWalletMeta => hwWalletMeta.Vendor === ts3Vendor && hwWalletMeta.Model === ts3Model);
}

function isTrezorSafe5Wallet(conceptualWallet: IConceptualWallet): boolean {
  ...
  return isHwKind(conceptualWallet, hwWalletMeta => hwWalletMeta.Vendor === ts5Vendor && hwWalletMeta.Model === ts5Model);
}
export function isLedgerNanoWallet(conceptualWallet: IConceptualWallet): boolean {
  const vendor = Config.wallets.hardwareWallet.ledgerNano.VENDOR;
  return isHwKind(conceptualWallet, hwWalletMeta => hwWalletMeta.Vendor === vendor);
}
