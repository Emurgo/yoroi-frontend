...
import { RemovePublicDeriver, ModifyConceptualWallet, DeleteAllTransactions } from '../../database/walletTypes/core/api/write';
...
import { GetAddress, GetPathWithSpecific, GetTransaction, GetCertificates } from '../../database/primitives/api/read';
...
|}): Promise<void> {
  ...
  return await raii<PromisslessReturnType<typeof removePublicDeriver>>(
    ...
    async dbTx => {
      ...
      await deps.ModifyAddress.remove(db, dbTx, walletAddressIds);

      // 3) remove utxos
      await ModifyUtxoAtSafePoint.remove(db, dbTx, publicDeriver.getPublicDeriverId());
      await ModifyUtxoDiffToBestBlock.removeAll(db, dbTx, publicDeriver.getPublicDeriverId());

      await deps.RemovePublicDeriver.remove(db, dbTx, { publicDeriverId: publicDeriver.getPublicDeriverId() });
