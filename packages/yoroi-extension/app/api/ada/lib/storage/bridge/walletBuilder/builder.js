...
import type { lf$Transaction, lf$Database } from 'lovefield';

import type { ConceptualWalletInsert } from '../../database/walletTypes/core/tables';
import { ModifyConceptualWallet } from '../../database/walletTypes/core/api/write';
import type { Cip1852WrapperInsert } from '../../database/walletTypes/cip1852/tables';
import { ModifyCip1852Wrapper } from '../../database/walletTypes/cip1852/api/write';
import type { AddAdhocPublicDeriverRequest } from '../../database/walletTypes/common/api/write';
import {
...
import { getAllTables, raii } from '../../database/utils';
import type { IDerivePublicFromPrivateRequest } from '../../models/ConceptualWallet/interfaces';
import { derivePublicDeriver } from '../../models/ConceptualWallet/traits';
...
export class WalletBuilder<CurrentState: { ... }> {
  ...
  updateData<Requirement: { ... }, NewAddition>(
  ...
  ): WalletBuilder<CurrentState & NewAddition & Requirement> {
    ...
    return new WalletBuilder<CurrentState & NewAddition & Requirement>(
      this.db,
      this.txHolder,
  ...
  static start(db: lf$Database, derivationTables: Map<number, string>): WalletBuilder<$Shape<{||}>> {
  ...
  async commit(): Promise<CurrentState> {
    // lock used tables
    const allTables = Array.from([...this.tables, ...Array.from(this.derivationTables.values())]);
    const usedTables = allTables.map(name => this.db.getSchema().table(name));

    await raii<void>(this.db, usedTables, async tx => {
      this.txHolder.tx = tx;
  ...
  > = (insert: CurrentState => ConceptualWalletInsert) => {
    return this.updateData<{ ... }, HasConceptualWallet>(
      ...
      async finalData => {
        finalData.conceptualWalletRow = await ModifyConceptualWallet.add(this.db, this.txHolder.tx, insert(finalData));
      }
    );
  };
  ...
  > = (insert: CurrentState => Cip1852WrapperInsert) => {
    return this.updateData<HasConceptualWallet, HasCip1852Wrapper>(
      ...
      async finalData => {
        finalData.cip1852WrapperRow = await ModifyCip1852Wrapper.add(this.db, this.txHolder.tx, insert(finalData));
      }
    );
  };
  ...
  > = <Insert>(request: CurrentState => AddAdhocPublicDeriverRequest<Insert>) => {
  ...
  > = (
    ...
    return this.updateData<HasConceptualWallet, HasPublicDeriver<mixed>>(
      ...
      async finalData => {
        const { privateDeriverKeyDerivationId, privateDeriverLevel, deriverRequest } = request(finalData);
        finalData.publicDeriver = [
          ...finalData.publicDeriver,
