...
import { legacyStorageKeys } from './database/legacy';
...
export async function migrateToLatest(localStorageApi: LocalStorageApi, persistentDb: lf$Database): Promise<boolean> {
 ...
 */
export async function cardanoTxHistoryReset(persistentDb: lf$Database): Promise<boolean> {
...
async function removeLedgerDevices(persistentDb: lf$Database): Promise<boolean> {
  const wallets = await loadWalletsFromStorage(persistentDb);
  if (wallets.length === 0) {
    return false;
  }
...
export async function populateNewUtxodata(persistentDb: lf$Database): Promise<boolean> {
  const wallets = await loadWalletsFromStorage(persistentDb);
  if (wallets.length === 0) {
    return false;
  }
  ...
  for (const publicDeriver of wallets) {
    try {
      ...
      const blocks = await raii<$ReadOnlyArray<$ReadOnly<BlockRow>>>(db, getAllSchemaTables(db, GetBlock), tx =>
        GetBlock.byIds(db, tx, blockIds)
      );
      // block ID => block height
      const blockMap = new Map<number, number>(blocks.map(block => [block.BlockId, block.Height]));
      ...
      await raii<void>(db, getAllSchemaTables(db, ModifyUtxoAtSafePoint), tx =>
        ModifyUtxoAtSafePoint.addOrReplace(db, tx, publicDeriver.getPublicDeriverId(), {
...
async function migrateWalletOrderAndSelectedWalletForNetworkSwitch(
  ...
  const selectedWallet = wallets.find(wallet => wallet.getPublicDeriverId() === selectedWalletId);
  if (selectedWallet) {
