...
import { DROPBOX_CLIENT_ID, MAX_MEMO_SIZE } from '../../../config/externalStorageConfig';
...
import type {
  ...
  GetMetadataExternalTxMemoRequest,
  ...
  GetMetadataExternalTxMemoResponse,
...
export default class DropboxApi implements IProvider {
  ...
  getMetadata: GetMetadataExternalTxMemoRequest => Promise<GetMetadataExternalTxMemoResponse>;
  async getMetadata(path: GetMetadataExternalTxMemoRequest): Promise<GetMetadataExternalTxMemoResponse> {
    const self = this;
    return await this.api
      ...
      .then(response => {
        if (!Object.prototype.hasOwnProperty.call(response, '.tag') || !Object.prototype.hasOwnProperty.call(response, 'name')) {
          throw new Error('Should never happen');
        }
        return {
          tag: response['.tag'],
          lastUpdated:
            Object.prototype.hasOwnProperty.call(response, 'server_modified') &&
  ...
  async fetchFolder(request: FetchFolderExternalTxMemoRequest): Promise<FetchFolderExternalTxMemoResponse> {
    ...
    const fullPath = walletId != null ? this.baseFolderPath.concat('/').concat(walletId) : this.baseFolderPath;
    ...
    return await this.api
      ...
      .then(response => {
        return response.entries
          .filter(
            entry => Object.prototype.hasOwnProperty.call(entry, '.tag') && Object.prototype.hasOwnProperty.call(entry, 'name')
          )
          .map(entry => {
            return {
              ...
              lastUpdated:
                Object.prototype.hasOwnProperty.call(entry, 'server_modified') &&
                // $FlowFixMe[incompatible-use]
