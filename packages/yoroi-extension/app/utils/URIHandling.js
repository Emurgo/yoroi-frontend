...
export const getURIParameters: (
  ...
  number,
  DefaultTokenEntry
) => Promise<?UriParams> = async (uri, currencyValidator, amountValidator, decimalPlaces, defaultTokenInfo) => {
...
const extractAddress: (string, (string) => Promise<boolean>) => Promise<?string> = async (uri, currencyValidator) => {
...
const extractAmount: (string, number, (string) => boolean) => ?BigNumber = (uri, decimalPlaces, amountValidator) => {
  // consider use of URLSearchParams
 ...
 */
export const buildURI = (address: string, amount: BigNumber): string => {
