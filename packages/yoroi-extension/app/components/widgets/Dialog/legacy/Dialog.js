...
function LegacyDialog(props: Props): Node {
  const {
    title,
    children,
    actions,
  ...
  return (
    <Modal
      ...
      sx={{
        bgcolor: 'var(--yoroi-comp-dialog-overlay-background-color)',
        display: 'flex',
      ...
      >
        ...
        {children != null ? (
          <ModalContent
            ...
            pb={!hasActions ? '24px' : '0px !important'}
...
export const ModalContainer: any => Node = styled(Box)(({ theme, empty = false }) => {
  const normalMinWidth = 'var(--yoroi-comp-dialog-min-width-md)';
  return {
    position: 'relative',
...
export default (withLayout(observer(LegacyDialog)): ComponentType<Props>);
