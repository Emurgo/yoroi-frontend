...
export default class AddNFTDialog extends Component<Props, State> {
  ...
  componentDidMount(): void {
    ...
    const selectedTokens = plannedTxInfoMap.filter(({ token }) => token.IsNFT).map(({ token }) => ({ token, included: true }));
      ...
      >
        <div className={styles.component}>
          <Box sx={{ position: 'relative', width: '100%' }}>
            <Box sx={{ position: 'absolute', top: '55%', left: '10px', transform: 'translateY(-50%)' }}>
              {' '}
              <SearchIcon />{' '}
            </Box>
            <OutlinedInput
          ...
          {currentNftsList.length === 0 ? (
            <div className={styles.noAssetFound}>
              ...
              <Typography variant="h1" className={styles.text} color="ds.text_gray_medium">
                {intl.formatMessage(fullNftsList.length === 0 ? messages.noNFTsYet : messages.noNFTsFound)}
              </Typography>
            </div>
          ) : (
            <>
              <div className={styles.nftsGrid}>
                {currentNftsList.map(nft => {
                  ...
                  return (
                    ...
                    >
                      <NFTImage image={nft.image ?? null} name={nft.name} width="141px" height="141px" />
                      <Typography
                        component="div"
                        variant="body1"
