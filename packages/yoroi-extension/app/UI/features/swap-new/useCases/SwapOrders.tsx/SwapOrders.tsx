...
import { ordersResultMock, primaryTokenInfo, tokensListMock } from "./mock";
...
export const SwapOrders = (props: Props) => {
  ...
  // const { swapForm: { orders } } = useSwapRevamp();
  ...
  const completedOrders = ordersResultMock.filter(order => order.status !== 'open'); // TODO: using real orders
  const openOrders = ordersResultMock.filter(order => order.status === 'open'); // TODO: using real orders
...
const OrderRow = ({
  ...
  const tokenOut = tokensListMock[order.tokenOut] // TODO: using real tokens
  const tokenIn = tokensListMock[order.tokenIn] // TODO: using real tokens
  ...
  return (
    <>
      <AssetPair
        sx={{ py: '20px' }}
        tokenIn={tokensListMock[order.tokenIn]}
        tokenOut={tokensListMock[order.tokenOut]}
        defaultTokenInfo={defaultTokenInfo}
      />
      ...
      {<Box textAlign="left">{ order.placedAt ? intl.formatDate(new Date(order.placedAt), { dateStyle: 'medium', timeStyle: 'short' }): null }</Box>}
