...
export default class WalletBackupStore extends Store<StoresMap> {
  ...
  @action initiateWalletBackup: ({| recoveryPhrase: Array<string>, name: string, password: string |}) => void = params => {
  ...
  @action addWordToWalletBackupVerification: ({| word: string, index: number |}) => void = params => {
  ...
  @action clearEnteredRecoveryPhrase: void => void = () => {
    this.enteredPhrase = [];
    this.recoveryPhraseSorted = this.recoveryPhraseSorted.map(({ word }) => ({ word, isActive: true }));
  };
