...
export default class UiNotificationsStore<TStores> extends Store<TStores> {
  @observable activeNotifications: Array<Notification> = [];

  @computed get mostRecentActiveNotification(): ?Notification {
    return this.activeNotifications.length > 0 ? this.activeNotifications[this.activeNotifications.length - 1] : null;
  }

  isOpen: string => boolean = (id: string): boolean => !!this._findNotificationById(id);

  getTooltipActiveNotification: string => ?Notification = (tooltipNotificationId: string): ?Notification => {
    let notification = null;
    const activeNotificationId = this.mostRecentActiveNotification ? this.mostRecentActiveNotification.id : '';
  ...
  _findNotificationById: string => ?Notification = id => this.activeNotifications.find(notification => notification.id === id);

  @action open: Notification => void = newNotification => {
    const notification: Notification = {
      ...newNotification,
      secondsTimerInterval: newNotification.duration != null ? setInterval(this._updateSeconds, 1000, newNotification.id) : null,
    };
