...
type SubRequestType = ({| publicDeriver: WalletState |}) => Promise<$ReadOnlyArray<$ReadOnly<StandardAddress>>>;
...
export class AddressTypeStore {
  ...
  constructor(data: {| stores: StoresMap, request: SubRequestType |}) {
  ...
  @action refreshAddressesFromDb: WalletState => Promise<void> = async publicDeriver => {
    const allRequest = this._getRequest(publicDeriver.publicDeriverId);
    allRequest.invalidate({ immediately: false });
    await allRequest.execute({ publicDeriver }).promise;
  };

  _flowCoerceResult: (CachedRequest<SubRequestType>) => ?$ReadOnlyArray<$ReadOnly<StandardAddress>> = request => {
  ...
  @action addObservedWallet: WalletState => void = ({ publicDeriverId }) => {
...
export class AddressBookSubgroup extends AddressTypeStore implements IAddressTypeStore {
  constructor(data: SubgroupCtorData): IAddressTypeStore {
    super({
      stores: data.stores,
      request: request =>
        data.stores.addresses._wrapForeign({
          ...request,
          storeName: data.name,
        }),
    });
...
export class ByronAllAddressesSubgroup extends AddressTypeStore implements IAddressTypeStore {
  constructor(data: SubgroupCtorData): IAddressTypeStore {
    super({
      stores: data.stores,
      request: request =>
        data.stores.addresses._wrapForAllAddresses({
          ...request,
          storeName: data.name,
...
export class ByronExternalAddressesSubgroup extends AddressTypeStore implements IAddressTypeStore {
  constructor(data: SubgroupCtorData): IAddressTypeStore {
    super({
      stores: data.stores,
      request: request =>
        data.stores.addresses._createAddressIfNeeded({
          publicDeriver: request.publicDeriver,
          genAddresses: () =>
            data.stores.addresses._wrapForChainAddresses({
              ...request,
              storeName: data.name,
              type: CoreAddressTypes.CARDANO_LEGACY,
...
export class EnterpriseExternalAddressesSubgroup extends AddressTypeStore implements IAddressTypeStore {
  constructor(data: SubgroupCtorData): IAddressTypeStore {
    super({
      stores: data.stores,
      request: request =>
...
export class EnterpriseInternalAddressesSubgroup extends AddressTypeStore implements IAddressTypeStore {
  constructor(data: SubgroupCtorData): IAddressTypeStore {
    super({
      stores: data.stores,
      request: request =>
