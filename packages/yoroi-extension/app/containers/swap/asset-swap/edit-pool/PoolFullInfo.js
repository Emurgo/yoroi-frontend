...
export default function SwapPoolFullInfo({ defaultTokenInfo, withInfo, showMinAda }: Props): React$Node {
  ...
  return (
    <Box sx={{ display: 'flex', flexFlow: 'column', gap: '8px' }}>
      {showMinAda && (
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box color="grayscale.600" display="flex" alignItems="center" gap="8px">
            <Typography color="ds.text_gray_normal">Min ADA</Typography>
            {withInfo && (
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box color="grayscale.600" display="flex" alignItems="center" gap="8px">
          <Typography color="ds.text_gray_normal">Fees</Typography>
          {withInfo && (
            <InfoTooltip
              content={
                <>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box color="grayscale.600" display="flex" alignItems="center" gap="8px">
          <Typography color="ds.text_gray_normal">Minimum assets received</Typography>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box color="grayscale.600" display="flex" alignItems="center" gap="8px">
          <Typography color="ds.text_gray_normal">Liquidity provider fee</Typography>
          {withInfo && (
            <InfoTooltip
              content={
