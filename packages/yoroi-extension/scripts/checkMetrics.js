...
const EXCLUDE = ['isInitializedAndEnabled', 'load', 'identify', 'track', 'flush'];

const eventNames = Object.keys(Ampli.prototype).filter(name => !EXCLUDE.includes(name));

const regExpStr = `ampli\\.(${eventNames.join('|')})\\(`;

const grepOutput = childProcess
  .execSync(`grep -h -o -r --include='*.js' -E '${regExpStr}' app chrome | sort | uniq`)
  .toString('utf8');

const seenEventNames = grepOutput
  .trim()
  .split('\n')
