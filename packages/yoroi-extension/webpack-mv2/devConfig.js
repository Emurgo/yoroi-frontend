...
const baseDevConfig = (networkName: string, isNightly: boolean, isLight: ?boolean = false, isE2E: ?boolean = false): * => ({
  ...
  entry: {
    yoroi: [customPath, hotScript, path.join(__dirname, '../chrome/extension/index')],
    background: [customPath, hotScript, path.join(__dirname, '../chrome/extension/background/index')],
    connector: [customPath, hotScript, path.join(__dirname, '../chrome/extension/connector/index')],
    ledger: [customPath, hotScript, path.join(__dirname, '../ledger/index')],
  },
  devMiddleware: {
...
const contentScriptConfig = (networkName: string, isNightly: boolean, isLight: ?boolean = false, isE2E: boolean): * => ({
  mode: 'development',
  resolve: commonConfig.resolve(),
  devtool: 'source-map',
  entry: {
