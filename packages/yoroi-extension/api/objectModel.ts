...
abstract class ModelHelper {
  ...
  async call(this: Model, path, ...args) {
    ...
    if (value instanceof MutateFunc) {
      return value.func.call(value.self, path.slice(0, -1), (eventPath, newValue = new AbsentParameter()) => {
...
class BasicModel extends ModelHelper implements Model {
  ...
  async getValue(path, options = { cache: false }) {
    const ROOT_KEY = 'root';
    let value = { [ROOT_KEY]: this.dataSource };
...
*/
export function extendProperty<ArgsT extends any[], RetT>(deps: ArgsT, func: (...args: ArgsT) => RetT): RetT {
...
function _makeAccessor(model: Model, path: Path = []): any {
  return new Proxy(
    {},
    {
      get(_target, prop) {
...
type GetUserType<SourceT> =
  SourceT extends Lazy<infer R>
    ? GetUserType<R>
    : SourceT extends (...args: infer ArgsT) => Promise<infer RetT>
...
export function makeAccessor<T>(dataSource: T): GetUserType<T> {
  return _makeAccessor(new BasicModel(dataSource));
}

export function makeClientAccessor<T>(requestServer: RequestServerFunc): {
  modelAccessor: T;
  onServerEvent: (event: Event) => void;
} {
...
export function makeAccessorServer(accessor: unknown, emitEvent: (event: Event) => void) {
...
type BetterPrint<T> = T extends infer O ? { [P in keyof O]: O[P] } : never;
...
export function extend<UserType, ExtenderType>(
  ...
  const { model, path } = accessor[_GET_ACCESSOR_META];
  const extendedAccessor = _makeAccessor(new ExtendedModel(model, path, extender));
