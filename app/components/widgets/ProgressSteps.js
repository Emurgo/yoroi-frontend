...
export default class ProgressSteps extends Component<Props> {
  ...
  createSetps = (stepsList, progressInfo): Array<Element> => {
    ...
    for (let idx = 0; idx < stepsList.length; idx++) {
      ...
      const textClass = isClassicThemeActive ? styles.stepTextClassic : styles.stepText;
      const topBarClass = isClassicThemeActive ? styles.stepTopBarClassic : styles.stepTopBar;
        ...
        <div key={idx} className={styles.stepBlock}>
          ...
          <div className={styles.stepBottomBlock}>
            <div className={styles.stepStateIconContainer}>
              {(displayIcon === 'done') && <SvgInline svg={isClassicThemeActive ? iconTickSVG : iconTickGreenSVG} />}
              {(displayIcon === 'error') && <SvgInline svg={isClassicThemeActive ? iconCrossSVG : iconCrossGreenSVG} />}
            </div>
